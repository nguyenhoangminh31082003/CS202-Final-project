...
private:
	...
	float cur_frame_time; // calculate current frame's time to determine if it is time to switch frame
public:
	Animation();
	Animation(const std::vector<sf::IntRect>& frames, float duration);
